...
class Expander{
  ...
  void expand(ArrayList<Expander> _exs){
    if(ar < 1){
      float dw = 1;
...
boolean judgeRect(float _x, float _y, float _w, float _h, ArrayList<Expander> _exs){
...
void removeLowerRanker(ArrayList<Expander> _exs, int _num){
...
int calcLifetime(float _w, float _h){
  return floor(_w * _h / 10);
}

void createExpander(ArrayList<Expander> _exs){
...
float calcFilllingRate(ArrayList<Expander> _exs){
  float sum = 0;
  for(Expander tmpex : _exs){
    sum += tmpex.getSize();
  }
...
void colorByTangency(ArrayList<Expander> _exs){
  // preparation
...
int getUnusedGroup(ArrayList<Expander> _exs){
  HashSet<Integer> hash_set = new HashSet<Integer>();
  for(Expander ex : _exs){
